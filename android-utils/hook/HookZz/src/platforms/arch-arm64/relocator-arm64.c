...
static bool arm64_relocator_rewrite_ADR(ARM64Relocator *self, const ARM64InstructionCTX *insn_ctx) {
    ARM64AssemblyrWriter *writer = self->output;
    uint32_t Rd;
    zz_addr_t immlo, immhi, label = 0;
    zz_addr_t target_address;

    Rd             = get_insn_sub(insn_ctx->insn, 0, 5);
    immhi          = (zz_addr_t )get_insn_sub(insn_ctx->insn, 5, 19);
    immlo          = (zz_addr_t )get_insn_sub(insn_ctx->insn, 29, 2);

    label = (immhi << 2) + immlo;

    if (immhi & 0x40000) {
        label = label | 0x0FFFFFFFFFFF80000;
    }

    if (get_insn_sub(insn_ctx->insn, 31, 1) == 1) {
        target_address = (label << 12) + (insn_ctx->pc & 0xFFFFFFFFFFFFF000);
    } else {
        target_address = label + insn_ctx->pc;
    }
 
    arm64_writer_put_ldr_reg_imm(writer, Rd, 0x8);
    arm64_writer_put_b_imm(writer, 0xc);
    arm64_relocator_register_literal_insn(self, writer->insnCTXs[self->output->insnCTXs_count]);
    arm64_writer_put_bytes(writer, (zz_ptr_t)&target_address, sizeof(target_address));
    return TRUE;
