...
DXRetStatus dx_hook_hookfun(void *symbol, void *replace, void **origin) {
    ...
    int machine_abe = utils::get_loaded_lib_abi("/linker");

    if (HKABI_I386 == machine_abe) {
        if (l_call_from_apk) {
    ...
    } else if (HKABI_ARM == machine_abe) {
#if defined(__arm__)
        if (!utils::is_hookable(symbol)) {
            return DX_SIZE_NOT_ENOUGH;
        }
#endif
    }
