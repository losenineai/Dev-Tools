...
// 8B 1C 24: 8B - 00 011 100 - 00 100 100
_disused static int GetPcTrunkRegIndex(uint8_t* buf) {
    hde64s decode = {0};
    hde64_disasm(buf, &decode);

    // printf("GetPcTrunkRegIndex:\n");
    // printf("    opcode: 0x%.2X\n", decode.opcode);
    // printf("    modrm_mod: 0x%.2X\n", decode.modrm_mod);
    // printf("    modrm_rm: 0x%.2X\n", decode.modrm_rm);
    // printf("    sib: 0x%.2X\n", decode.sib);
    // printf("    buf[3]: 0x%.2X\n", buf[3]);
    // printf("    modrm_reg: 0x%.2X\n", decode.modrm_reg);

    if (decode.opcode == 0x8B
        && (decode.modrm_mod == 0x0 && decode.modrm_rm == 0x4)
        && decode.sib == 0x24
        && buf[3] == 0xC3 
        && decode.modrm_reg != 4 && decode.modrm_reg != 5) {
        return decode.modrm_reg;
    }
    return -1;
}
