...
//    LOGE("anti_multirun_check: data: %s", socket_data->data);
    ...
    dx_anti_pthread_create(ret, &newthread, NULL, anti_multirun_start_server, socket_data);
...
void *DXAnti_detect_multirun(void *arg) {
    ...
    } else if(anti_multirun_xiaomi_check() || anti_multirun_socket_check()){
        err_code = _multirun_crash_code;
    }
...
int DXAnti_detect_multirun_async() {
    ...
    dx_anti_pthread_create(ret, &newthread, NULL, DXAnti_detect_multirun, NULL);
