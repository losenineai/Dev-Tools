...
extern int subprocessPid;
extern uint32_t get_random_Pid();
extern int TEST_PTRACE;
/**
 * 未完成：自身进程名检测
...
bool anti_debug_detectByObjProcess() {
    ...
    // 获取结果
    while (dx_fgets(buf, sizeof(buf), pfile)) {
        // 打印进程
        // anti_info_log("遍历进程:%s\n",buf);
        // 查找子串
        char *strA = dx_strstr(buf, "android_server");
        char *strB = dx_strstr(buf, "gdbserver");
        char *strC = dx_strstr(buf, "gdb");
        char *strD = dx_strstr(buf, "fuwu");
        char *strE = dx_strstr(buf, "frida");
...
bool anti_debug_detectByParents() {
    ...
    if (pProcessName.size() > 0) {
        // 未匹配，返回0
        if (dx_strstr(pProcessName.c_str(), "zygote") == NULL) {
            result = true;
        }
    }
...
bool anti_debug_detectByNativeIsDBGConnected() {
    ...
    bool result = false;
    void *handle = NULL;
    handle = dx_dlopen("/system/lib/libdvm.so", RTLD_LAZY);

    if (NULL == handle) {
        return result;
    }

    PPP Fun = (PPP) dx_dlsym(handle, "_Z25dvmDbgIsDebuggerConnectedv");
...
bool anti_debug_detectByTrace() {
    ...
    for (string &line : lineVect) {
        if (!line.empty() && line.find("TracerPid:") != line.npos) {
            ...
            anti_info_log("anti_debug_detectByTrace tpid:%d, subprocessPid:%d", tpid, subprocessPid);

            if (tpid != 0 && tpid != subprocessPid) {
                result = true;
                break;
            }
        }
...
int DXAnti_detect_debug_inner_async(pthread_t &tid) {
    if(ptrace(TEST_PTRACE, get_random_Pid(), NULL, NULL) == get_random_Pid() + getpid()){
        return 0;
    }

    int ret = -1;

    dx_anti_pthread_create(ret, &tid, NULL, DXAnti_detect_debug_inner, NULL);
