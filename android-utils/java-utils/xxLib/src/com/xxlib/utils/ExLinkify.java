...
public class ExLinkify {
     ...
     */
    public static final boolean addLinks(Spannable text, int mask, int color) {
        if (mask == 0) {
            return false;
        }
        ...
        for (LinkSpec link: links) {
            applyLink(color, link.url, link.start, link.end, text);
        }
     ...
     *  LinkMovementMethod.
     */
    public static final boolean addLinks(TextView text, int mask, int color) {
        if (mask == 0) {
            return false;
        }
        ...
        if (t instanceof Spannable) {
            if (addLinks((Spannable) t, mask, color)) {
     ...
     */
    public static final void addLinks(int color, TextView text, Pattern pattern, String scheme) {
        addLinks(color, text, pattern, scheme, null, null);
    }
     ...
     */
    public static final void addLinks(int color, TextView text, Pattern p, String scheme,
                                      MatchFilter matchFilter, TransformFilter transformFilter) {
        SpannableString s = SpannableString.valueOf(text.getText());

        if (addLinks(color, s, p, scheme, matchFilter, transformFilter)) {
            text.setText(s);
            addLinkMovementMethod(text);
        }
    }
     ...
     *                      a scheme specified in the link text
     */
    public static final boolean addLinks(int color, Spannable text, Pattern pattern, String scheme) {
        return addLinks(color, text, pattern, scheme, null, null);
    }
     ...
     *                      to be converted into links.
     */
    public static final boolean addLinks(int color, Spannable s, Pattern p,
        ...
        while (m.find()) {
            ...
            if (allowed) {
                ...
                applyLink(color, url, start, end, s);
                hasMatches = true;
            }
        }
    ...
    private static final void applyLink(int color, final String url, int start, int end, Spannable text) {
