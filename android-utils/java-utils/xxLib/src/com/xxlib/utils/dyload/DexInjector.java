...
public class DexInjector {
    ...
    private static synchronized Boolean injectAboveEqualApiLevel14(
        ...
        DexClassLoader dexClassLoader = new DexClassLoader(dexPath, defaultDexOptPath, nativeLibPath, pathClassLoader);
        try {
            Class<?> dummyClass = dexClassLoader.loadClass(dummyClassName);
            LogTool.i(TAG, null == dummyClass? "dummyClass is null" : dummyClass.getName());
            ...
            Log.i(TAG, "--> injectAboveEqualApiLevel14 ex: " + e.toString());
            e.printStackTrace();
            return false;
        }
