...
public class CheckAppStateUtils {
    ...
    private static final String TAG = "CheckAppStateUtils";

    // top命令
    public static final String[] TOP = { "/system/bin/top", "-n", "1", "-d", "0" };

    private static boolean IS_DEBUG = CheckAppRunning.isDebugRunning;
    public static int GAME_PID = -1;
        ...
        if (result != null && !result.trim().equals("")) {
            for (String runResult : resultRoot) {
                ...
                if (GAME_PID == -1) {
                    ...
                    try {
                        GAME_PID = Integer.parseInt(pid);
                    } catch (Throwable t) {
                        LogTool.w(TAG, t.toString());
                    }
                    if (IS_DEBUG)
                        LogTool.i(TAG, "GAME_PID " + GAME_PID);
                } else {
                    pid = String.valueOf(GAME_PID);
                }
    ...
    public static int getMiniOomAdjPid(List<Integer> pids) {
        if (null == pids || pids.size() == 0) {
            if (IS_DEBUG) {
                LogTool.i(TAG, "pids is null or empty");
            }
            return 0;
        }
        int minOomAdjPid = 0;
        ...
        for (Integer pid : pids) {
            ...
            if (IS_DEBUG) {
                LogTool.i(TAG, "pid " + pid + " oom_adj is " + oomAdjTmp);
            }
    ...
    protected static int getOomAdjByPid(String pid) {
        ...
        try {
            if (IS_DEBUG) {
                LogTool.i(TAG, " getOomAdjByPid pid " + pid);
            }
            checkRunningResult = readConsole("cat /proc/" + pid + "/oom_adj", false);
            if (IS_DEBUG)
                LogTool.i(TAG, "oom_adj value:" + (checkRunningResult == null ? "null" : checkRunningResult));
            checkRunningResult.trim();
            int valueOfOOMAdj = Integer.valueOf(checkRunningResult);
            if (IS_DEBUG)
                LogTool.i(TAG, "valueOfOOMAdj : " + valueOfOOMAdj);
            return valueOfOOMAdj;
        } catch (Throwable e) {
            if (IS_DEBUG)
                LogTool.i(TAG, "getOomAdjByPid excption" + e.toString());
            // e.printStackTrace();
            return 999;
        }
    }
    ...
    public static boolean isCanGetProcessOomadj() {
        ...
        LogTool.i(TAG, "initOomadj " + initOomadj);
