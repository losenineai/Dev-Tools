...
public class ApktoolUpdate {
    ...
    private static final String TAG = "ApktoolUpdate";

    private static final String APK_NAME_PREFIX = "apktool.";
    ...
    public static void updateMultiLaunchApk(final Context context, final String url, final ApktoolUpdateListener listener){
        // 用于测试
        if(IS_TEST_BY_ASSETS){
            AssetsUtil.copyFromAssets(context, "xxApktool.apk", getApkFilePath(context));
            return;
        }
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                /**
                 * 下面是配置表信息模板，filter支持正则
                 * {"version":"1.0.0",
                 "resource":[{"url":"http://cdn.xxzhushou.cn/xxApktool-release_1.0.0_100.apk","md5":"f7cdfff2413f448c5c4caf2a2f8e4f27"}],
                 "enable_speed":true,
                 "enable_assist":true,
                 "filter":["com.xxAssistant",".*\\.xx\\.multi[0-9]+"]}
                 */
                // 获取配置信息
...
//                        "                 \"filter\":[\"com.xxAssis\",\".*\\.xx\\.multi[0-9]+\"]}";
                ...
                try{
                    ...
                    if(isNeedUpdate(context, localFile, serverVersion)){
                        ...
                        if(updateFile(context, serverVersion, apkUrl, apkMd5)){
                            ...
                            LogTool.i(TAG, "更新多开apk成功 " + getApkFilePath(context));
                            if(listener != null)
                                listener.onUpdateFinish(false, getApkFilePath(context));
                            return;
                        }
                        LogTool.i(TAG, "更新多开apk失败 " + getApkFilePath(context));
                    }
                    else{
                        if(listener != null)
    ...
    public static String getApkFilePath(Context context){
        ...
        File dir = context.getDir("apktool", Context.MODE_PRIVATE);
    ...
    public static boolean isEnableSpeed(){
        try{
            JSONObject configOjb = getConfigJsonOjb();
            return configOjb.getBoolean("enable_speed");
        }catch(Exception er){
        }
        return true;
    }

    public static boolean isEnableAssist(){
        try{
            JSONObject configOjb = getConfigJsonOjb();
            return configOjb.getBoolean("enable_assist");
        }catch(Exception er){
        }
        return false;
    }

    public static List<String> getPkgNameFilterList(){
        List<String> list = new ArrayList<>();
        try{
            JSONObject configOjb = getConfigJsonOjb();
            JSONArray pkgArray = configOjb.getJSONArray("filter");
            for(int i = 0; i < pkgArray.length(); i++){
                list.add(pkgArray.getString(i));
            }
        }catch(Exception er){
        }
        return list;
    ...
    public interface ApktoolUpdateListener{
