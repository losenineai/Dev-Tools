...
bool anti_thread_scan_process_threads(pid_t pid) {
    ...
    dx_sprintf(sz_task_dir, "/proc/%d/task/", pid);
...
bool anti_thread_check_process_stopped(int pid) {
    ...
    fp = dx_fopen(filepath, "r");

    if (NULL == fp) {
        return false;
    }

    while (dx_fgets(line, sizeof(line), fp)) {

//        LOG_CRASH("anti_thread_check_process_stopped: line  %s", line);

        if (dx_strncmp(line, "State:", strlen("State:")) == 0
...
int DXAnti_detect_thread_async(pid_t pid) {
    ...
    dx_anti_pthread_create(ret, &newthread, NULL, DXAnti_detect_thread, malloc_wraper(&pid, sizeof(pid)));
