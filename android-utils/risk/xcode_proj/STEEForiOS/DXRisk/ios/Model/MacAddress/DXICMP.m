...
+ (void)sendICMPEchoRequestTo: (nonnull NSString *)ipAddress {
    ...
    icmp->icmp_code = 0;
    uint32_t as = 0;
    SecRandomCopyBytes(kSecRandomDefault, 1, &as);
    uint32_t ab = 0;
    SecRandomCopyBytes(kSecRandomDefault, 1, &ab);
    icmp->icmp_seq = as;
    icmp->icmp_id = ab;
    icmp->icmp_cksum = 0;
    ...
    dest.sin_addr.s_addr = [DXAddress IPv4Pton:ipAddress];
