...
#define BUFLEN (sizeof(struct rt_msghdr_) + 512)
...
struct rt_msghdr_ {
    ...
    struct rt_metrics_ rtm_rmx; /* metrics themselves */
};
#endif /* _NET_ROUTE_H_ */
...
+ (nullable NSString *)walkMACAddressOf: (nonnull NSString *)ipAddress {
    ...
    in_addr_t searchIpAddress = [DXAddress IPv4Pton:ipAddress];
    
    lim = buf + needed;
    for(next = buf; next < lim; next += rtm->rtm_msglen) {
        rtm = (struct rt_msghdr_ *)next;
        sin2 = (struct sockaddr_inarp *)(rtm + 1);
        sdl = (struct sockaddr_dl *)((char *)sin2 + SA_SIZE(sin2));
        if(searchIpAddress == sin2->sin_addr.s_addr) {
            macAddress = [DXAddress linkLayerNtop:sdl];
            break;
        }//end if found
    }//end for
    free(buf);
...
+ (nullable NSString *)MACAddressOf: (nonnull NSString *)ipAddress {
    ...
    struct rt_msghdr_ *rtm = (struct rt_msghdr_ *)buf;
    rtm->rtm_msglen = sizeof(struct rt_msghdr_) + sizeof(struct sockaddr_in);
    ...
    sin->sin_addr.s_addr = [DXAddress IPv4Pton:ipAddress];
    ...
    NSString *macAddress = nil;
    if (n != 0) {
        struct sockaddr_dl *sdl = (struct sockaddr_dl *)(buf + sizeof(struct rt_msghdr_) + sizeof(struct sockaddr_inarp));
        macAddress = [DXAddress linkLayerNtop:sdl];
    }//end if
    return macAddress;
}//end MACAddressOf:
