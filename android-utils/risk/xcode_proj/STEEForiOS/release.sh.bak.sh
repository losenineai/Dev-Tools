...
xcodebuild -toolchain dx-vm${CI_TEST_TOOLCHAIN_SUFFIX} -target ${MODULE} -sdk $SDKVERSION $ARCH_IPHONEOS -configuration Release >> $LOGLOCATION || build_error
...
xcodebuild -toolchain dx-vm${CI_TEST_TOOLCHAIN_SUFFIX} -target ${MODULE} -sdk $SDKVERSION_S $ARCH_SIMULATOR -configuration Release >> $LOGLOCATION || build_error
...
xcodebuild -toolchain dx-vm${CI_TEST_TOOLCHAIN_SUFFIX} -target ${MODULE}WithIDFA -sdk $SDKVERSION $ARCH_IPHONEOS -configuration Release >> $LOGLOCATION || build_error
...
xcodebuild -toolchain dx-vm${CI_TEST_TOOLCHAIN_SUFFIX} -target ${MODULE}WithIDFA -sdk $SDKVERSION_S $ARCH_SIMULATOR -configuration Release >> $LOGLOCATION || build_error
...
if [ "$MODULE" == "STEESafeCheckForiOS" ]; then
cp "${DEVICE_DIR_INCLUDE}/DXSafeManager.h" "$OUTDIR/header"
cp "${DEVICE_DIR_INCLUDE}/DXSafeManager.h" "$OUTDIR/${DYMODULE}.framework/Headers"
fi
echo "Done"
...
elif [ "$MODULE" == "STEESafeCheckForiOS" ]; then
PACKAGENAME="libDXSafe"
fi

lipo -create "$SRCROOT/build/Release-iphoneos/lib${MODULE}.a" "$SRCROOT/build/Release-iphonesimulator/lib${MODULE}.a" -output "$OUTDIR/${PACKAGENAME}.a"
...
if [ "$MODULE" == "STEERiskMgrForiOS" ]; then
lipo -create "$SRCROOT/build/Release-iphoneos/lib${MODULE}WithIDFA.a" "$SRCROOT/build/Release-iphonesimulator/lib${MODULE}WithIDFA.a" -output "$OUTDIR/${PACKAGENAME}-with-idfa.a"
...
STEE_IDFA_A="${PACKAGENAME}-with-idfa.a"
STEE_IDFA_DY="${DYMODULE}WithIDFA.framework"
STEE_IDFA_ST="${DYMODULE}WithIDFA.framework"
else
STEE_IDFA_A=""
STEE_IDFA_DY=""
STEE_IDFA_ST=""
...
zip -r $TARGET_FILE header ${PACKAGENAME}.a $STEE_IDFA_A ${DYMODULE}.framework $STEE_IDFA_DY ${FRAMEWORK_MODULE}.framework $STEE_IDFA_ST
