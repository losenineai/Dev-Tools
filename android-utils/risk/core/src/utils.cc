...
string CryptUtils::AES_Decrypt(const string &key_, const string &src) {
    ...
    if (src.length() % STEP != 0) {
        LOGE("aes decrypt input length is %d (%%%d != 0)", src.length(), STEP);
        return "";
    }
...
//    string report_url = url + "?sign=" + sign + "&appKey=" + appkey + "&app=" + get_pkg_name();
    ...
    string respone_data = sync_http_request(HTTP_POST, report_url, encrypt_zip_data, isSaas);
...
int64_t OthersUtils::CurrentTimesBySec() {
    struct timeval tv;
    gettimeofday(&tv, NULL);
...
STEE
string OthersUtils::CreateSTEERequestRaw(STEEDataType type, const string &appId, const map<string, string> &data_map){

    size_t n_indos_size = data_map.size();
    if (n_indos_size == 0) {
        return OthersUtils::CreateSTEERequestRaw(type, appId, ProtobufCBinaryData{});
    }
    ...
    string request_str = OthersUtils::CreateSTEERequestRaw(type, appId, binaryData);
...
string OthersUtils::ExecuteCMD(const string &cmd, const string &filter) {
    ...
    dxFILE *pipe = dx_popen(cmd.c_str(), "r");
    if (pipe != NULL) {
        char buf[512];
        while (!dx_feof(pipe)) {
            if (dx_fgets(buf, sizeof(buf), pipe) != NULL) {
                ss += buf;
            }
        }
        dx_pclose(pipe);
    ...
    if (D != NULL) {
        ...
        while ((ent = readdir(D)) != NULL) {
            ...
            for (auto i = filters.begin(); i != filters.end(); i++) {
                if (dx_strstr(ent->d_name, i->c_str()) != NULL) {
                    skip = true;
                    break;
                }
            }
...
string StringUtils::ToHexString(const string &input) {
    ...
    for (int i = 0; i < input.size(); i++) {
        dx_sprintf(buf + i * 2, "%02x", (uint8_t) input.at(i));
    }
...
#ifdef __aarch64__
    dx_sprintf(pStr, "%li", value);
#else
    dx_sprintf(pStr, "%lli", value);
#endif
...
#ifdef __aarch64__
    dx_sprintf(str, "%li", value);
#else
    dx_sprintf(str, "%lli", value);
#endif
