...
void *dx_dlopen(const char *libpath, int flags) {
    ...
    size_t dynstr = 0, dynsym = 0;
    size_t dynstrsz = 0;
    size_t dynsym_fake_end = 0;
    size_t loadBias = 0;

    get_sym_in_dynamic(elf, dynsym, dynstr, dynstrsz, dynsym_fake_end, loadBias);

    if(dynsym == 0 || dynstr == 0 || dynstrsz == 0) {
        free(ctx);
        return NULL;
    }
    ...
    ctx->dynstr_limit = (void*)((size_t)ctx->dynstr + dynstrsz);
    ctx->loadBias = loadBias;

    ctx->nsym = (dynsym_fake_end - dynsym)/sizeof(Elf_Sym);
    return ctx;
}
