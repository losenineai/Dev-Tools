...
void DXAnti_detect_debug(pid_t main_pid) {
    ...
    dx_pthread_create(ret, &newthread, NULL, anti_debug_check_thread, NULL);
...
void* anti_debug_keep_pipe_connect(void* arg) {
    ...
    do {
        errno = 0;
    } while ( read(fildes, &buf, 1u) == -1 && errno == EAGAIN );
    close(fildes);
...
void *DXAnti_detect_debug(void *arg){
    ...
    if ( fork_pid > 0 )
    {
        ...
        if ( read(fildes_sub_to_parent[0], &buf, 1u) != -1 && buf == 0)
        {
            ...
            dx_pthread_create(ret, &newthread, NULL, anti_debug_keep_pipe_connect, (void* )pair);
        }
...
int DXAnti_detect_debug_async() {
    ...
    dx_pthread_create(ret, &tid, NULL, DXAnti_detect_debug, NULL);
