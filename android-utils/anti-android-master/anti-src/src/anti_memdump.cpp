...
static bool l_add_thread_flag = false;
...
void anti_memdump_add_process_watch(pid_t pid) {
    ...
    sprintf(sz, "/proc/%d/mem", pid);
    watcher_add(sz);
    l_process_paths.push_back(sz);

    sprintf(sz, "/proc/%d/pagemap", pid);
    watcher_add(sz);
    l_process_paths.push_back(sz);
}
...
void anti_memdump_add_thread_watch(pid_t pid, pid_t tid) {
    ...
    sprintf(sz, "/proc/%d/task/%d/mem", pid, tid);
    watcher_add(sz);
    if (l_auto_rm_switch_on) {
        l_thread_paths.push_back(sz);
    }

    sprintf(sz, "/proc/%d/task/%d/pagemap", pid, tid);
    watcher_add(sz);
    if (l_auto_rm_switch_on) {
        l_thread_paths.push_back(sz);
    }
...
STEE
static void anti_memdump_stop_monitor() {
    l_add_thread_flag = false;
}

STEE
void * anti_memdump_monitor_pid(void* arg) {

    anti_info_log("anti_memdump_monitor_pid");
    pid_t pid = *(pid_t* )arg;
    char sz[256] = {0};
    ...
    /**
     * SIGUSR1 信号 会与React Native冲突
     */
//    struct sigaction act = {};
//    act.sa_handler = anti_memdump_stop_monitor();
//    sigaction(SIGUSR1, (const struct sigaction *)&act, NULL);

    sprintf(sz, "/proc/%d/task/", pid);
    l_add_thread_flag = true;
    while ( l_add_thread_flag )
    {
        struct dirent* dir;
        DIR* dirp = NULL;
        while (dirp == NULL) {
        ...
        while ( (dir = readdir(dirp)) != NULL && l_add_thread_flag)
        {
            
            const char *dirname = &dir->d_name[0];

            anti_info_log("readdir(dirp) dirname:%s", dirname);
            if ( 0 != strcmp(dirname, ".") && 0 != strcmp(dirname, "..") )
            {
                struct stat status = {0};
                char tid_file[0x100] = {0};

                sprintf(tid_file, "%s%s", sz, dirname);
        ...
        if (pass_time >= auto_rm_time && l_add_thread_flag) {
...
void *DXAnti_detect_memdump(void *arg) {
    ...
    if ( watcher_init() )
    {
        ...
        dx_pthread_create(ret, &newthread, NULL, anti_memdump_monitor_pid, malloc_wraper((void* )&pid, sizeof(pid)));

        watcher_dispose();
        anti_memdump_stop_monitor();
...
int DXAnti_detect_memdump_async(pthread_t &tid, pid_t pid) {
    ...
    dx_pthread_create(ret, &tid, NULL, DXAnti_detect_memdump, malloc_wraper((void* )&pid, sizeof(pid)));
