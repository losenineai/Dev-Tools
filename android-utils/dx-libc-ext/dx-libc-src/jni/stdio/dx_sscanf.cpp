...
static int dx_eofread(void *cookie, char *buf, int len)
{
...
{
	dxFILE f;
	struct __dxsfileext fext;

	_FILEEXT_SETUP(&f, &fext);
	f._flags = __SRD;
	f._bf._base = f._p = (unsigned char *)str;
	f._bf._size = f._r = dx_strlen(str);
	f._read = dx_eofread;
	f._lb._base = NULL;
	return (__dxsvfscanf(&f, fmt, ap));
}


int dx_sscanf(const char* s, const char* fmt, ...) {
  PRINTF_IMPL(dx_vsscanf(s, fmt, ap));
}
...
int __dxsvfscanf(dxFILE* fp, const char* fmt0, va_list ap) {
  ...
  for (;;) {
    ...
    if (IsSpace(c)) {
      while ((fp->_r > 0 || __dxsrefill(fp) == 0) && IsSpace(*fp->_p)) nread++, fp->_r--, fp->_p++;
      continue;
    }
    if (c != '%') goto literal;
    width = 0;
...
again:
    ...
    switch (c) {
      case '%':
literal:
        if (fp->_r <= 0 && __dxsrefill(fp)) goto input_failure;
    ...
    switch (c) {
      case CT_CHAR:
        ...
        if (flags & LONG) {
          ...
          size_t r = dx_fread(p, 1, width, fp);
      ...
      case CT_INT:
         ...
         */
        if (flags & NDIGITS) {
          if (p > buf) (void)dx_ungetc(*(u_char*)--p, fp);
          goto match_failure;
        }
        c = ((u_char*)p)[-1];
        if (c == 'x' || c == 'X') {
      ...
      case CT_FLOAT:
        ...
        if (width == 0 || width > sizeof(buf) - 1) width = sizeof(buf) - 1;
        if ((width = dx_parsefloat(fp, buf, buf + width)) == 0) goto match_failure;
        if ((flags & SUPPRESS) == 0) {
          if (flags & LONGDBL) {
