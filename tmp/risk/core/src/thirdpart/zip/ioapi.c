...
static voidpf file_build_ioposix(FILE *file, const char *filename)
{
    ...
    if (file == NULL)
        return NULL;
    ioposix = (FILE_IOPOSIX*)calloc(1,sizeof(FILE_IOPOSIX));
    ioposix->file = file;
    ioposix->filenameLength = strlen(filename) + 1;
    ioposix->filename = (char*)calloc(ioposix->filenameLength, sizeof(char));
...
static voidpf ZCALLBACK fopendisk64_file_func (voidpf opaque, voidpf stream, int number_disk, int mode)
{
    ...
    diskFilename = (char*)calloc(ioposix->filenameLength , sizeof(char));
