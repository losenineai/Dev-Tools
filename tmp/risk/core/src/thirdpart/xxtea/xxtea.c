...
STEE
void * xxtea_encrypt_in(const void * data, size_t len, const void * key, size_t * out_len) {
    FIXED_KEY
    return xxtea_ubyte_encrypt((const uint8_t *)data, len, fixed_key, out_len);
}

STEE
void * xxtea_decrypt_in(const void * data, size_t len, const void * key, size_t * out_len) {
    FIXED_KEY
