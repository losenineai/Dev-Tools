...
static const char *getIdentstr(unsigned *size, int small)
{
    ...
    "$Id: dx-protector "
    UPX_VERSION_STRING4
    " Copyright (C) 2017-" UPX_VERSION_YEAR " Dingxiang Technologies Co., Ltd. All Rights Reserved. $"
    "\n";
    static char identsmall[] =
    "\n"
    "$Id: dx-protector "
    "(C) 2017-" UPX_VERSION_YEAR " Dingxiang Technologies Co., Ltd. All Rights Reserved. https://www.dingxiang-inc.com $"
    "\n";
    static char identtiny[] = UPX_VERSION_STRING4;

    ///////////////////////////////////////////////////////////////////////////////
    static char _identbig[] =
    "\n\0"
    "$Info: "
    "so-protector $"
    "\n\0"
    "$Id: so-protector "
    UPX_VERSION_STRING4
    " Copyright (C) 2017-" UPX_VERSION_YEAR
    "\n";
    static char _identsmall[] =
    "\n"
    "$Id: so-protector "
    "(C) 2017-" UPX_VERSION_YEAR
    "\n";
    static char _identtiny[] = UPX_VERSION_STRING4;
    ////////////////////////////////////////////////////////////////////////////////
    
    //LOGD("identbig_size : %d", identbig_size);
    static int done;
    if (!done && (opt->debug.fake_stub_version[0] || opt->debug.fake_stub_year[0]))
    {   
        struct strinfo_t { char *s; int size; };
        strinfo_t* strlist = NULL;
        if(opt->replace_identify == NULL){
            static strinfo_t _strlist[] = {
            ...
            static strinfo_t _strlist[] = {
            { _identbig,   (int)sizeof(_identbig) },
            { _identsmall, (int)sizeof(_identsmall) },
            { _identtiny,  (int)sizeof(_identtiny) },
            { NULL, 0 } };
            strlist = _strlist;
        }
    ...
    if(opt->replace_identify == NULL){
        ...
        if (small >= 2)
        {
            *size = sizeof(_identtiny);
            return _identtiny;
        }
        else if (small >= 1)
        {
            *size = sizeof(_identsmall);
            return _identsmall;
        }
        else
        {
            *size = sizeof(_identbig);
            return _identbig;
        }
    }
