...
off_t PackLinuxElf32::pack3(OutputFile *fo, Filter &ft)
{
    ...
    if (0!=xct_off) {  // shared library
        ...
        unsigned pre_ptload_endaddr = 0;
        unsigned int pt2th_soslide = 0;
        int ptloadindex = 0;
        for (unsigned j = 0; j < e_phnum; ++j, ++phdr) {
            unsigned const len  = get_te32(&phdr->p_filesz);
            //unsigned const len  = get_te32(&phdr->p_memsz);
            unsigned const ioff = get_te32(&phdr->p_offset);
            unsigned       align= get_te32(&phdr->p_align);
            unsigned const type = get_te32(&phdr->p_type);
            //printf("index:%d, orig type:%d\n", j, (int)phdr->p_type);
            //printf("index:%d, type:%d\n", j, type);
            //printf("index:%d, flag:%d\n", j, (int)phdr->p_flags);
            //PT_LOAD && RW second PT_LOAD SEGMENT
            if (phdr->p_type == Elf32_Phdr::PT_LOAD && phdr->p_flags == 6) {
                unsigned page_align = ~(0x1000-1);
                if ((v_hole & page_align) == ((unsigned)phdr->p_vaddr & page_align)) {
                    printf("v_hole = %X, vaddr = %X\n", v_hole, (unsigned)phdr->p_vaddr);
                    throw CantPackException("not enough space to save loader!");
                }
            }
            /*
            ...
            */
            if (PT_LOAD32==type) {
                ...
                if (xct_off < ioff) {  // Slide up non-first PT_LOAD.
                    ...
                    if(off < pre_ptload_endaddr){
                        off += ((int)((pre_ptload_endaddr-off)/align)+1) * align;
                    }
                    pre_ptload_endaddr = off + len;
                    ...
                    if(pt2th_soslide == 0){
                        pt2th_soslide = so_slide;
                    }
                    set_te32(&phdr->p_offset, so_slide + ioff);
                }
                else {  // Change length of first PT_LOAD.
                    set_te32(&phdr->p_filesz, sz_pack2 + lsize);
                    //修复throw时，会对当前pc指针做判断，判断其是否在so的ptload1所在mem范围之内。
                    if(opt->o_unix.android_shlib){
                        set_te32(&phdr->p_memsz, phdr->p_memsz + 0x1000);
                    }else{
                        set_te32(&phdr->p_memsz,  sz_pack2 + lsize);
                    }
                }
                if(opt->o_unix.expire_time != 0 && opt->o_unix.android_shlib){
                    if(Elf32_Ehdr::EM_ARM==e_machine){
                        if(word & 1){
                            word -= 1;
                        }
                        update_expire_time(fo, 
                            stub_arm_v5t_linux_shlib_init, 
                            sizeof(stub_arm_v5t_linux_shlib_init),
                            word);
                    }else if(Elf32_Ehdr::EM_386==e_machine){
                        update_expire_time(fo, 
                            stub_i386_linux_shlib_init, 
                            sizeof(stub_i386_linux_shlib_init),
                            word);
                    }
                }
            ...
            if (xct_off < ioff)
                set_te32(&phdr->p_offset, pt2th_soslide + ioff);
        }  // end each Phdr
        ...
        if (opt->o_unix.android_shlib) {
            ...
            for (unsigned j = 0; j < e_shnum; ++shdr, ++j) {
                ...
                if (Elf32_Shdr::SHT_DYNAMIC == get_te32(&shdr->sh_type)) {
                    ...
                    set_te32(&shdr->sh_offset, pt2th_soslide + offset );
                ...
                &&  !strcmp(".rel.plt", get_te32(&shdr->sh_name) + shstrtab)) {
                    ...
                    fo->seek(pt2th_soslide + f_off, SEEK_SET);  // FIXME: assumes PT_LOAD[1]
...
bool PackLinuxElf::update_expire_time(OutputFile *fo, 
        const unsigned char* upxloader, 
        unsigned int loader_length, 
        unsigned int init_addr)
{
    struct _LOADER_MAGIC {
        unsigned char EXPIRETIME[4];
    } loader_magic={
        {0x06, 0xDE, 0xAD, 0xDE}
    };
    off_t off_a = 0;
    ...
    if (Elf32_Ehdr::EM_386 ==e_machine) {
        off = 0x34;
    } else if(Elf32_Ehdr::EM_ARM ==e_machine) {
        off = 0x34;
    } else if(Elf64_Ehdr::EM_AARCH64 == e_machine){
        off = 0x40;
    }

    off_a = search_bytes((const char*)upxloader, loader_length, (const char*)loader_magic.EXPIRETIME, sizeof(loader_magic.EXPIRETIME));
        if(off_a == -1){
        throwCantUnpack("can not find expire time...");
        return false;
    }

    off_a -= off;
    
    fo->seek(init_addr+off_a, SEEK_SET);
    fo->rewrite(&(opt->o_unix.expire_time), sizeof(loader_magic.EXPIRETIME));
            ...
            */
            if (PT_LOAD64 == type) {
                ...
                if (xct_off < ioff) {  // Slide up non-first PT_LOAD.
                    ...
                    //修复throw时，会对当前pc指针做判断，判断其是否在so的ptload1所在mem范围之内。
                    if(opt->o_unix.android_shlib){
                        set_te64(&phdr->p_memsz, phdr->p_memsz + 0x1000);
                    }else{
                ...
                if(opt->o_unix.expire_time != 0 && opt->o_unix.android_shlib){
                    if(Elf64_Ehdr::EM_AARCH64 == e_machine){
                        update_expire_time(fo, 
                            stub_arm64_linux_shlib_init, 
                            sizeof(stub_arm64_linux_shlib_init),
                            word);
                    }
                }
            ...
            if (xct_off < ioff)
                set_te64(&phdr->p_offset, pt2th_soslide + ioff);
        }  // end each Phdr
        ...
        if (opt->o_unix.android_shlib) {
            ...
            for (unsigned j = 0; j < e_shnum; ++shdr, ++j) {
                ...
                if (Elf64_Shdr::SHT_DYNAMIC == sh_type) {
                    ...
                    set_te64(&shdr->sh_offset, pt2th_soslide + offset);
...
bool PackLinuxElf32::canPack()
{
    ...
    if (Elf32_Ehdr::ET_DYN==get_te16(&ehdr->e_type)) {
        ...
        if (/*jni_onload_sym || elf_find_dynamic(upx_dt_init) ||*/true) {
            ...
            if (e_shnum) {
                for (int j= e_shnum; --j>=0; ++shdr) {
                    ...
                    // }
                    if (upx_dt_init != Elf32_Dyn::DT_INIT)
                        upx_dt_init = 0;
...
PackLinuxElf64::canPack()
{
    ...
    if (Elf64_Ehdr::ET_DYN==get_te16(&ehdr->e_type)) {
        ...
        if (/*elf_find_dynamic(upx_dt_init)*/true) {
                ...
                if (xct_off <= offset) { // above the extra page
                    if (PT_NOTE32 == get_te32(&phdr->p_type)) {
                        ...
                        if (sizeof(buf_notes) < (memsz + len_notes)) {
                            throwCantPack("PT_NOTES too big3");
                        }
            ...
            for (unsigned j = 0; j < e_shnum; ++j, ++shdr) {
                ...
                if (Elf32_Shdr::SHT_RELA== sh_type) {
                    ...
                    for (int k = sh_size / sh_entsize; --k >= 0; ++rela) {
                        ...
                        if (Elf32_Ehdr::EM_ARM == e_machine || Elf32_Ehdr::EM_386 == e_machine) {
...
void PackLinuxElf64::pack1(OutputFile *fo, Filter & /*ft*/)
{
    ...
    if (0!=xct_off) {  // shared library
        ...
        if (opt->o_unix.android_shlib) {
            ...
            // Relocate Phdr virtual addresses, but not physical offsets and sizes
            unsigned char buf_notes[4096]; memset(buf_notes, 0, sizeof(buf_notes));
            unsigned len_notes = 0;
            ...
            for (unsigned j = 0; j < e_phnum; ++j, ++phdr) {
                ...
                if (xct_off <= offset) { // above the extra page
                    if (PT_NOTE64 == get_te32(&phdr->p_type)) {
                        ...
                        if (sizeof(buf_notes) < (memsz + len_notes)) {
                            throwCantPack("PT_NOTES too big2");
                        }
            ...
            for (unsigned j = 0; j < e_shnum; ++j, ++shdr) {
                ...
                if (Elf64_Shdr::SHT_RELA == sh_type) {
                    ...
                    for (int k = sh_size / sh_entsize; --k >= 0; ++rela) {
                        ...
                        if (Elf64_Ehdr::EM_AARCH64 == e_machine || Elf64_Ehdr::EM_X86_64 == e_machine) {
                            if (R_AARCH64_RELATIVE == r_type || R_X86_64_RELATIVE == r_type) {
                                if (xct_off <= r_addend) {
                                    set_te64(&rela->r_addend, asl_delta + r_addend);
                                }
                            }
                            if (R_AARCH64_JUMP_SLOT == r_type || R_X86_64_JUMP_SLOT == r_type) {
                                // .rela.plt contains offset of the "first time" target
                                if (plt_off > r_offset) {
                                    plt_off = r_offset;
                                }
                ...
                if (Elf64_Shdr::SHT_REL == sh_type) {
                    ...
                    for (int k = sh_size / sh_entsize; --k >= 0; ++rel) {
                        ...
                        unsigned r_type = ELF64_R_TYPE(r_info);
                        if (xct_off <= w
                        &&  (Elf64_Ehdr::EM_AARCH64 == e_machine || Elf64_Ehdr::EM_X86_64 == e_machine)
                        &&  (  R_AARCH64_RELATIVE  == r_type
                            || R_AARCH64_JUMP_SLOT == r_type 
                            || R_X86_64_RELATIVE == r_type
                            || R_X86_64_JUMP_SLOT == r_type)) {
                            set_te64(&file_image[d], asl_delta + w);
                        }
                    }
                }
...
void PackLinuxElf32::pack4(OutputFile *fo, Filter &ft)
{
    ...
    if (0!=xct_off) {  // shared library
        ...
        this->data_encrypt(&linfo, sizeof(linfo));
...
void PackLinuxElf64::unpack(OutputFile *fo)
{
    ...
    this->data_decrypt(&linfo, sizeof(linfo));
    ...
    this->data_decrypt(&hbuf, sizeof(hbuf));
...
PackLinuxElf32::check_pt_dynamic(Elf32_Phdr const *const phdr)
{
    ...
    LOGD("t:%ld, s:%ld, vaddr:%ld, filesz:%ld, memsz:%ld",t, s, vaddr, filesz, memsz);
