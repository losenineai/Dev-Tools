...
#define V(x)    (strcmp(UPX_VERSION_STRING, UPX_VERSION_STRING4) ? UPX_VERSION_STRING : UPX_VERSION_STRING x)
    ...
    if(opt->replace_identify == NULL){
        con_fprintf(f,
                "                       DingXiang Ultimate Protector\n"
                "                        Copyright (C) 2017 - " UPX_VERSION_YEAR "\n"
                "                     Dingxiang Technologies Co., Ltd.\n\n");
        fg = con_fg(f,fg);
    }else{
        //std::string msg = "NULL";
        const char* format_msg = 
                ...
                "                       Copyright (C) 2017 - %s\n"
                "                     %s Technologies Co., Ltd.\n\n";
        size_t len = strlen(opt->replace_identify) + strlen(format_msg) + 5;
        char* com_msg = (char*)malloc(len);
        sprintf(com_msg, format_msg, opt->replace_identify , UPX_VERSION_YEAR, opt->replace_identify);
        fputs(com_msg, f);
        fg = con_fg(f, fg);
    }
...
//
**************************************************************************/
void show_taginfo(void){
    FILE *f = con_term;

    con_fprintf(f, "version:%s\n", TAG_VERSION);
    con_fprintf(f, "build time:%s\n", BUILD_TIME);
