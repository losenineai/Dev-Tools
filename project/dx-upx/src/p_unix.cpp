...
void PackUnix::pack(OutputFile *fo)
{
    ...
    this->data_encrypt(&hbuf, sizeof(hbuf));
...
void PackUnix::data_encrypt(void* data, int size){
    unsigned char* vdata = (unsigned char*)data;
    char key[2] = {'d', 'x'}; 
...
void PackUnix::data_decrypt(void* data, int size){
    unsigned char* vdata = (unsigned char*)data;
    char key[2] = {'d', 'x'}; 
    for(int i=0; i<size; i++){
        vdata[i] ^= key[1];
