...
template <class T>
static int getopt_unsigned_var(T *var, const T min_value, const T max_value, const char *arg_fatal)
{
    const char *p = mfx_optarg;
    char *endptr;
    int r = 0;
...
static int do_option(int optc, const char *arg)
{
    ...
    case 910:
    {
        ...
        if (!mfx_optarg || !mfx_optarg[0])
            e_optarg(arg);
        size_t len = strlen(mfx_optarg);
        opt->license_name = (char *)malloc(len + sizeof(char));

        memcpy((void*)opt->license_name, mfx_optarg, len);
        opt->license_name[len] = '\0';
        break;
    }
    ...
    case 677:
        ...
        break;
    case 678:
        getopt_unsigned_var(&opt->o_unix.android_signhash, 1u, ~0u, arg);
        break;
    case 679:
        getopt_unsigned_var(&opt->o_unix.android_debug_sighash, 1u, ~0u, arg);
        break;
    case 680:
        opt->o_unix.anti_ida = true;
        break;
    case 691:
    {
        //设置授权文件路径
        if (!mfx_optarg || !mfx_optarg[0])
            e_optarg(arg);
        size_t len = strlen(mfx_optarg);
        opt->replace_identify = (char *)malloc(len + sizeof(char));
        memcpy((void*)opt->replace_identify, mfx_optarg, len);
        opt->replace_identify[len] = '\0';        
        break;
    }
    case 692:
    {
        if (!mfx_optarg || !mfx_optarg[0])
            e_optarg(arg);
        size_t len = strlen(mfx_optarg);
        opt->orig = (char *)malloc(len + sizeof(char));
        memcpy((void*)opt->orig, mfx_optarg, len);
        opt->orig[len] = '\0';        
        break;
    }
    case 693:
    {
        if (!mfx_optarg || !mfx_optarg[0])
            e_optarg(arg);
        size_t len = strlen(mfx_optarg);
        opt->report = (char *)malloc(len + sizeof(char));
        memcpy((void*)opt->report, mfx_optarg, len);
        opt->report[len] = '\0';        
        break;
    }
    case 694:
    {
        if (!mfx_optarg || !mfx_optarg[0])
            e_optarg(arg);
        size_t len = strlen(mfx_optarg);
        opt->pic = (char *)malloc(len + sizeof(char));
        memcpy((void*)opt->pic, mfx_optarg, len);
        opt->pic[len] = '\0';        
        break;
    }
    case 681:
        opt->o_unix.android_check_integrity = true;
        break;
    case 682:
        opt->o_unix.fix_sdk_build = true;
        break;
    case 683:
        opt->o_unix.android_anti_dump = true;
        break;
    case 684:
        opt->o_unix.encrypt_shellcode = true;
        break;
    case 685:
        opt->o_unix.android_anti_debug = true;
        break;
    case 686:
        getopt_unsigned_var(&opt->o_unix.expire_time, 1u, ~0u, arg);
        break;
    case 687:
        opt->o_unix.expand_space = true;
        break;
    case '\0':
        return -1;
    case ':':
...
static const struct mfx_option longopts[] =
{
    ...
    {"force-pie",           0, 0, 677},
    {"android-check-sign",  0x31, 0, 678},
    {"android-check-debug-sign",  0x31, 0, 679},
    {"anti-ida",            0, 0, 680},
    {"replace-identify", 0x21, 0, 691},
    {"orig",         0x21, 0, 692},
    {"report",      0x21, 0, 693},
    {"pic",          0x21, 0, 694},
    {"android-check-integrity", 0, 0, 681},
    {"fix-sdk-build", 0, 0, 682},
    {"android-anti-dump", 0, 0, 683},
    {"encrypt-shellcode", 0, 0, 684},
    {"android-anti-debug", 0, 0, 685},
    {"expire-time",         0x31, 0, 686},
    {"expand-space", 0x21, 0, 687},
    // watcom/le
...
{
    int i;
    static char default_argv0[] = "dxprotector";
...
//    cmdline_cmd = opt->cmd;
    ...
    opt->arg0 = (char*)malloc(4096);
    if(strlen(argv[0])>4096){
        LOGE("path is too long");
        return -1;
    }
    strncpy(opt->arg0, argv[0], strlen(argv[0]));

#if (ACC_OS_WIN32 || ACC_OS_WIN64) && (defined(__MINGW32__) || defined(__MINGW64__))
    char licensePath[MAX_PATH] = {};
    if (opt->license_name)
    {
        strncpy(licensePath, opt->license_name, sizeof(licensePath));
        if (opt->license_name != NULL)
        {
            free(opt->license_name);
            opt->license_name = NULL;
        }
    }
    else
    {
        GetModuleFileNameA(NULL, licensePath, MAX_PATH);
        //printf("license file path is %s\n", licensePath);
        strrchr(licensePath, '\\')[1] = '\0';
        strcat(licensePath, "license.key");
    }
    dxlic_init();
    dxlic_setLicenseFile(licensePath);
    if (!dxlic_checkLicenseValid()) {
        dxlic_release();
        if(opt->replace_identify == NULL){
            MessageBox (NULL, "license is invalid,please contact support@dingxiang-inc.com.", "dxprotector", MB_OK);
            printf("license is invalid,please contact support@dingxiang-inc.com.\n");
        }else{
            MessageBox (NULL, "license is invalid.", "soprotector", MB_OK);
...
#elif (ACC_OS_POSIX_LINUX) && (defined(__linux__) || defined(__LINUX__)) && 0
    char licensePath[PATH_MAX] = {};
    if (opt->license_name)
    {
        strncpy(licensePath, opt->license_name, sizeof(licensePath));
        if (opt->license_name != NULL)
    ...
    {
        int cnt = readlink("/proc/self/exe", licensePath, PATH_MAX);
...
//MACOSX
    ...
    else
    {
        ...
        snprintf(cmdline, PATH_MAX, "dirname %s", argv[0]);
	    file = popen(cmdline, "r");
        fgets(licensePath, PATH_MAX, file);
        int cnt = strlen(licensePath);
        if(cnt<0 || cnt >= PATH_MAX){
            return -1;
        }
        licensePath[cnt-1] = '/';
